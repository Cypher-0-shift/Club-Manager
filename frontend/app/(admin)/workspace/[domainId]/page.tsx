...
function DomainModal({ domain, onClose, onSuccess, onDelete }: { domain?: Domain; onClose: () => void; onSuccess: () => void; onDelete?: () => void }) {
  ...
  return (
    <div className="modal-overlay" onClick={onClose} style={{ background: 'rgba(0,0,0,0.85)' }}>
      <div className="modal-panel" style={{ width: '100%', maxWidth: '420px', background: '#262626', border: '1px solid rgba(255,255,255,0.08)', borderRadius: '10px' }} onClick={e => e.stopPropagation()}>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end' }}>
          <div>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: '14px' }}>
              ...
              <div>
                <h1 style={{ fontSize: '18px', fontWeight: 700, color: '#e5e5e5' }}>{domain?.name ?? 'Loading…'}</h1>
                ...
                {projects.map(p => {
                  ...
                  return (
                    <a key={p.id} href={`/board?project_id=${p.id}`} style={{ textDecoration: 'none', color: 'inherit' }}>
                      <div className="project-card" style={{ display: 'flex', flexDirection: 'column', gap: '14px', cursor: 'pointer', transition: 'all 0.2s', position: 'relative', minHeight: '180px', background: '#262626', border: '1px solid rgba(255,255,255,0.07)', borderRadius: '10px', padding: '16px' }}>
                        {canManage && (
                        ...
                        <div style={{ display: 'flex', gap: '8px', flexWrap: 'wrap' }}>
                          {['pending', 'in_progress', 'completed', 'overdue'].map(st => {
                            ...
                            return count > 0 ? (
                              <div key={st} style={{ display: 'flex', alignItems: 'center', gap: '4px', background: 'rgba(255,255,255,0.04)', padding: '2px 8px', borderRadius: '4px', border: '1px solid rgba(255,255,255,0.07)' }}>
