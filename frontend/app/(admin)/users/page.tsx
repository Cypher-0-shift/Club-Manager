  ...
  return (
    <AppShell>
      <div style={{ display: 'flex', flexDirection: 'column', gap: '24px' }}>
        <div>
          <h1 className="section-title" style={{ fontSize: '20px' }}>User Management</h1>
          <p className="section-subtitle">Manage organization access and roles</p>
        </div>

        {/* Tabs */}
        <div style={{ display: 'flex', gap: '8px', borderBottom: '1px solid var(--color-border-subtle)' }}>
          <button 
            className={`btn btn-ghost ${activeTab === 'pending' ? 'btn-primary' : ''}`} 
            onClick={() => setActiveTab('pending')}
            style={{ borderRadius: '4px 4px 0 0', paddingBottom: '12px', borderBottom: activeTab === 'pending' ? '2px solid var(--color-brand)' : 'none' }}
          >
          ...
          <button 
            ...
            style={{ borderRadius: '4px 4px 0 0', paddingBottom: '12px', borderBottom: activeTab === 'members' ? '2px solid var(--color-brand)' : 'none' }}
          >
            All Members
          </button>
          {isPresident && (
            <button 
              ...
              style={{ borderRadius: '4px 4px 0 0', paddingBottom: '12px', borderBottom: activeTab === 'roles' ? '2px solid var(--color-brand)' : 'none' }}
            >
              Roles & Permissions
            </button>
          )}
                      ...
                      {pending.map(u => {
                        ...
                        return (
                          <tr key={u.id}>
                            ...
                            <td><span style={{ textTransform: 'capitalize' }} className={`badge badge-${u.role === 'vp' || u.role === 'secretary' ? 'high' : 'low'}`}>{u.role}</span></td>
                        ...
                        <tr key={u.id}>
                          ...
                          <td>
                            {isPresident ? (
                              ...
                              <span style={{ textTransform: 'capitalize' }} className="badge badge-low">{u.role}</span>
                            )}
                          </td>
