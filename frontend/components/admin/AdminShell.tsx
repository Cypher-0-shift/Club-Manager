    ...
    <div style={{ display: 'flex', height: '100vh', overflow: 'hidden', background: 'var(--color-bg)' }}>
      ...
      >
        ...
        <nav style={{ flex: 1, padding: '16px 0', display: 'flex', flexDirection: 'column', gap: '4px' }}>
          ...
          {/* Workspace / Domains section */}
          <div>
            <div
              className={`nav-item ${isActive('/workspace') || !!projectId ? 'active' : ''}`}
              style={{ width: '100%', justifyContent: collapsed ? 'center' : 'space-between', borderRadius: 0, paddingRight: collapsed ? '0' : '8px' }}
          ...
          <a href="/board" className={`nav-item ${pathname === '/board' && !projectId ? 'active' : ''}`} style={{ justifyContent: collapsed ? 'center' : 'flex-start' }} title={collapsed ? 'Board' : undefined}>
            <KanbanSquare size={16} /> {!collapsed && <span style={{ overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>Board</span>}
          </a>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: '10px', justifyContent: collapsed ? 'center' : 'flex-start' }}>
              ...
              {!collapsed && (
                <div style={{ flex: 1, minWidth: 0 }}>
                  ...
                  <div style={{ fontSize: '11px', color: 'var(--color-text-muted)', textTransform: 'capitalize' }}>{user.role}</div>
                </div>
              )}
              {!collapsed && (
