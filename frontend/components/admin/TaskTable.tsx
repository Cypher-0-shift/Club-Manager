...
import { Search, ArrowUpDown, CheckCircle2, Circle, Clock, AlertCircle } from 'lucide-react';
...
type SortField = 'title' | 'status' | 'priority' | 'deadline' | 'assignee' | 'project';

/**
 * TaskTable — admin-only sortable/filterable task table view.
        ...
        <div style={{ overflowX: 'auto' }}>
          <table style={{ width: '100%', borderCollapse: 'collapse', textAlign: 'left', fontSize: '13px' }}>
            <thead style={{ background: 'rgba(255,255,255,0.03)', borderBottom: '1px solid var(--color-border-subtle)' }}>
              <tr>
                <th onClick={() => handleSort('title')} style={{ padding: '12px 16px', fontWeight: 600, color: 'var(--color-text-muted)', cursor: 'pointer' }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: '6px' }}>Task {sortField === 'title' && <ArrowUpDown size={12} />}</div>
                </th>
                <th onClick={() => handleSort('assignee')} style={{ padding: '12px 16px', fontWeight: 600, color: 'var(--color-text-muted)', cursor: 'pointer' }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: '6px' }}>Assignee {sortField === 'assignee' && <ArrowUpDown size={12} />}</div>
                </th>
                <th onClick={() => handleSort('project')} style={{ padding: '12px 16px', fontWeight: 600, color: 'var(--color-text-muted)', cursor: 'pointer' }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: '6px' }}>Project {sortField === 'project' && <ArrowUpDown size={12} />}</div>
                </th>
                <th onClick={() => handleSort('status')} style={{ padding: '12px 16px', fontWeight: 600, color: 'var(--color-text-muted)', cursor: 'pointer' }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: '6px' }}>Status {sortField === 'status' && <ArrowUpDown size={12} />}</div>
                </th>
                <th onClick={() => handleSort('priority')} style={{ padding: '12px 16px', fontWeight: 600, color: 'var(--color-text-muted)', cursor: 'pointer' }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: '6px' }}>Priority {sortField === 'priority' && <ArrowUpDown size={12} />}</div>
                </th>
                <th onClick={() => handleSort('deadline')} style={{ padding: '12px 16px', fontWeight: 600, color: 'var(--color-text-muted)', cursor: 'pointer' }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: '6px' }}>Deadline {sortField === 'deadline' && <ArrowUpDown size={12} />}</div>
                </th>
              </tr>
            </thead>
            <tbody>
                ...
                >
                  ...
                  <td style={{ padding: '14px 16px' }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                      <div className="avatar avatar-xs" style={{ fontSize: '10px', background: domainColorMap[t.assignee?.domain_id || ''] || 'var(--color-surface-hover)' }}>{(t.assignee?.full_name ?? 'U')[0]}</div>
                      <div style={{ display: 'flex', flexDirection: 'column' }}>
                        <span style={{ fontWeight: 500 }}>
                          {t.assignee?.full_name ?? 'Unassigned'}
                        </span>
                        {t.assignee && <span style={{ fontSize: '10px', color: 'var(--color-text-muted)', textTransform: 'capitalize' }}>{t.assignee.role}</span>}
                      </div>
                    </div>
                  </td>
