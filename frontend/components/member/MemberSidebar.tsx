...
export function MemberSidebar() {
  ...
  function getAvatarColor(id: string) { return ['#3a3a3a','#444444','#3d3d3d','#424242','#404040','#3e3e3e'][id.charCodeAt(0) % 6]; }
  ...
  return (
    <aside 
      ...
      style={{
        ...
        background: '#000000', borderRight: '1px solid rgba(255,255,255,0.07)',
    ...
    >
      {!collapsed && (
        <div onMouseDown={startResizing} style={{ position: 'absolute', right: '-2px', top: 0, bottom: 0, width: '4px', cursor: 'col-resize', zIndex: 100, background: isResizing ? 'rgba(255,255,255,0.2)' : 'transparent', transition: 'background 0.2s' }} />
      )}

      <div style={{ height: '56px', borderBottom: '1px solid rgba(255,255,255,0.07)', display: 'flex', alignItems: 'center', padding: collapsed ? '0 12px' : '0 16px', justifyContent: collapsed ? 'center' : 'space-between', gap: '10px', flexShrink: 0 }}>
        {!collapsed && (
      ...
      <nav style={{ flex: 1, padding: '16px 0', display: 'flex', flexDirection: 'column', gap: '4px' }}>
        <div style={{ padding: collapsed ? '0 8px' : '0 16px', marginBottom: '8px', textAlign: collapsed ? 'center' : 'left' }}>
          <span style={{ fontSize: '10px', fontWeight: 600, color: '#525252', textTransform: 'uppercase', letterSpacing: '0.05em' }}>{collapsed ? '•••' : 'Menu'}</span>
        </div>
        ...
        <div style={{ padding: collapsed ? '16px 8px 8px' : '16px 16px 8px', marginTop: 'auto', textAlign: collapsed ? 'center' : 'left' }}>
           <span style={{ fontSize: '10px', fontWeight: 600, color: '#525252', textTransform: 'uppercase', letterSpacing: '0.05em' }}>{collapsed ? '•••' : 'System'}</span>
        </div>
